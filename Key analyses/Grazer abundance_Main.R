# Grazer abundance_Main.R
#
#   Brighton experiment (2×2): Warming (Pad_Colour) × Pollution (Pad_Region)
#   Response: grazer presence/absence, derived from total of Gastropod_Count_ON + Gastropod_Count_SIDE
#
#   Outputs:
#     1) Per-date bias-reduced binomial GLMs (brglm2; logit)
#     2) GLM interaction classification table (±5% equivalence band on exp(beta_int); odds-ratio scale)
#     3) Whole-summer GLM-style visualisation (ggeffects predictions + raw overlay)
#     4) Conditional per-date panels (region-stratified lines/ribbons across pad colour)
#     5) Whole-season binomial GAM (mgcv::gam) with time smooths varying by Region only
#     6) GAM interaction classification table (parametric fixed effects; OR-scale interaction)
#     7) Whole-season GAM visualisation (smooth predictions + raw overlay)
#
#   Evidence/diagnostics (separation checks, alternative structures) are kept in:
#     Grazer abundance_Additional.R
...
#   - For logit models:
#       beta_int is on the log-odds scale, so exp(beta_int) is an interaction odds ratio (OR).
#   - We treat interactions as "additive" if the interaction OR CI overlaps ±5%: [0.95, 1.05], conditional on p < alpha.
P_ALPHA  <- 0.05
EQUIV_LO <- 0.95
EQUIV_HI <- 1.05
...
# Build the binary response:
#   - Aggregate ON + SIDE counts to avoid splitting into two fields and to supplement sparse counts for modelling
#   - Presence/absence avoids count sparsity and aligns with subsequent binary modelling
brighton_data <- brighton_data %>%
  mutate(
...
########## 2) Individual-date GLMs (bias-reduced binomial logit) ##########
...
#   - Bias-reduced binomial GLMs
glm_JUN_bi   <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = JUN,   family = binomial("logit"), method = "brglmFit")
glm_JUL_1_bi <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = JUL_1, family = binomial("logit"), method = "brglmFit")
glm_JUL_2_bi <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = JUL_2, family = binomial("logit"), method = "brglmFit")
glm_AUG_1_bi <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = AUG_1, family = binomial("logit"), method = "brglmFit")
glm_AUG_2_bi <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = AUG_2, family = binomial("logit"), method = "brglmFit")
glm_SEP_1_bi <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = SEP_1, family = binomial("logit"), method = "brglmFit")
glm_SEP_2_bi <- glm(Gastropod_Binary ~ Pad_Colour * Pad_Region, data = SEP_2, family = binomial("logit"), method = "brglmFit")

glm_models <- list(glm_JUN_bi, glm_JUL_1_bi, glm_JUL_2_bi, glm_AUG_1_bi, glm_AUG_2_bi, glm_SEP_1_bi, glm_SEP_2_bi)

########## 3) GLM interaction classification + summary table ##########

extract_grazer_glm_stats <- function(model, date_label) {
  
  # Coefficient table (logit scale):
  #   α    = baseline log-odds (Ambient + Non-Polluted)
  #   βw   = warming effect (log-odds change) in Non-Polluted
  #   βp   = pollution effect (log-odds change) under Ambient
  #   βint = interaction (log-odds deviation beyond additivity)
  ...
  # Additive vs observed on the logit scale
  Δadd_logit <- βw + βp
  Δobs_logit <- Δadd_logit + βint
  
  # Null/observed probabilities (back-transformed)
  null_prob <- plogis(α + Δadd_logit)
  obs_prob  <- plogis(α + Δobs_logit)
  
  # Interaction OR + CI (odds ratio scale)
  int_or    <- exp(βint)
  int_or_lo <- exp(int$conf.low)
  int_or_hi <- exp(int$conf.high)
  
  # Perspective metrics under each stressor stratum
  # Note: these are % changes in odds (not probability), consistent with logit interpretation.
  ...
  # Equivalence-band logic on OR scale (±5% around 1)
  ...
  dir_label <- dplyr::case_when(
    (Δadd_logit > 0 & Δobs_logit < 0) ~ "Reversal (Pos to Neg)",
    (Δadd_logit < 0 & Δobs_logit > 0) ~ "Reversal (Neg to Pos)",
    (βw > 0 & βp > 0 & βint > 0)      ~ "Synergism (>1)",
    (βw > 0 & βp > 0 & βint < 0)      ~ "Antagonism (<1)",
    (βw < 0 & βp < 0 & βint < 0)      ~ "Synergism (<1)",
    (βw < 0 & βp < 0 & βint > 0)      ~ "Antagonism (>1)",
    sign(Δadd_logit) == sign(βint)    ~ "Synergism (same–sign)",
  ...
  cs %>%
    ...
    dplyr::transmute(
      ...
      `ΔAdd (logit)`         = if_else(Term == "Warming × Pollution", round(Δadd_logit, 4), NA_real_),
      `ΔObs (logit)`         = if_else(Term == "Warming × Pollution", round(Δobs_logit, 4), NA_real_),
      `Null Probability`     = if_else(Term == "Warming × Pollution", round(null_prob, 4), NA_real_),
      `Observed Probability` = if_else(Term == "Warming × Pollution", round(obs_prob,  4), NA_real_),
      `Odds Ratio`           = if_else(Term == "Warming × Pollution", round(int_or,    4), NA_real_),
      `Interaction Ratio CI` = if_else(Term == "Warming × Pollution", sprintf("[%.3f, %.3f]", int_or_lo, int_or_hi), NA_character_),
...
grazer_glm_table <- purrr::map2_df(glm_models, DATE_LABELS, extract_grazer_glm_stats)

DT::datatable(
  grazer_glm_table,
  caption = "Grazer presence (bias-reduced binomial GLM, logit link): fixed effects & interaction summary",
...
# Plot 
line_plot_grazer <- ggplot(all_preds, aes(x = Sampling_Date, y = predicted, group = Treatment)) +
  ...
  geom_jitter(
    ...
    aes(x = Sampling_Date, y = Gastropod_Binary, color = Treatment),
    shape = 20, width = 0.25, height = 0.03, alpha = 0.7, size = 1.5, inherit.aes = FALSE
  ) +
  labs(
    title = "Grazer presence",
    x = "Sampling date",
    y = "Probability of grazer presence (%)"
  ) +
  theme_minimal() +
  theme(
...
create_conditional_plot <- function(model, date_label, include_legend = FALSE, italicize_p = TRUE) {
  
  # ggpredict gives predicted mean probability + CI at Ambient/Warm for each region
  ...
  ggplot(pr, aes(x = .data$x, y = .data$predicted, group = .data$Pad_Region)) +
    ...
    scale_y_continuous(limits = c(0, 1), labels = scales::percent, oob = scales::squish) +
...
plot_JUN   <- create_conditional_plot(glm_JUN_bi,   "June")
plot_JUL_1 <- create_conditional_plot(glm_JUL_1_bi, "July 1")
plot_JUL_2 <- create_conditional_plot(glm_JUL_2_bi, "July 2")
plot_AUG_1 <- create_conditional_plot(glm_AUG_1_bi, "August 1")
plot_AUG_2 <- create_conditional_plot(glm_AUG_2_bi, "August 2")
plot_SEP_1 <- create_conditional_plot(glm_SEP_1_bi, "September 1")
plot_SEP_2 <- create_conditional_plot(glm_SEP_2_bi, "September 2", include_legend = TRUE)
...
# Whole-season GAM (binomial, logit):
...
print(summary(grazer_gam_final))

########## 7) GAM interaction classification + summary table ##########

extract_grazer_gam_stats <- function(model, date_label = "Whole Season") {
  ...
  i_w   <- grep("pad[._ ]*colourb$", tlc)
  i_p   <- grep("pad[._ ]*regionp$", tlc)
  i_ix  <- grep("pad[._ ]*colourb[:*]pad[._ ]*regionp", tlc)
  ...
  null_prob <- plogis(α + Δadd)
  obs_prob  <- plogis(α + Δobs)
  ...
  # Perspective metrics (odds % change)
  w_pct  <- (exp(βw)      - 1) * 100
  wp_pct <- (exp(βw+βint) - 1) * 100
  p_pct  <- (exp(βp)      - 1) * 100
  pw_pct <- (exp(βp+βint) - 1) * 100
  
  ci_overlaps <- !(int_or_hi < EQUIV_LO || int_or_lo > EQUIV_HI)
  ci_outside  <-  (int_or_hi < EQUIV_LO || int_or_lo > EQUIV_HI)
  ...
  int_class <- dplyr::case_when(
    ...
    ci_overlaps                      ~ "Additive (CI overlaps ±5%)",
    ci_outside & pv_i < P_ALPHA      ~ dir_lab,
    TRUE                             ~ "Unclassified"
  )
  ...
  out <- tibble::tibble(
    ...
    `Null Probability`           = c(NA, NA, null_prob),
    `Observed Probability`       = c(NA, NA, obs_prob),
  ...
  out$`Null Probability`     <- ifelse(is.na(out$`Null Probability`), NA, round(out$`Null Probability`, 4))
  out$`Observed Probability` <- ifelse(is.na(out$`Observed Probability`), NA, round(out$`Observed Probability`, 4))
  out$`Odds Ratio`           <- ifelse(is.na(out$`Odds Ratio`), NA, round(out$`Odds Ratio`, 4))
  
  out
}

grazer_gam_table <- extract_grazer_gam_stats(grazer_gam_final, "Whole Season")

DT::datatable(
  grazer_gam_table,
  caption = "Grazer presence GAM (binomial logit; 2 smooths by region; + pad RE): parametric fixed effects & interaction summary",
  options = list(pageLength = 10, scrollX = TRUE)
)
...
# Predict on the link scale (logit), then back-transform to probabilities
pr_link <- predict(
  grazer_gam_final,
  newdata  = pred_grid,
  type     = "link",
...
graz_gam_time_plot <- ggplot(gam_preds, aes(x = Days_Since_Start, y = predicted, group = Treatment)) +
  ...
  geom_jitter(
    ...
    aes(x = Days_Since_Start, y = Gastropod_Binary, color = Treatment),
    shape = 20, width = 1.0, height = 0.03, alpha = 0.7, size = 1.5, inherit.aes = FALSE
  ) +
  ...
  scale_fill_manual(values = TREATMENT_COLORS,  breaks = LEGEND_ORDER) +
  labs(
    title = "Whole-season GAM: grazer presence",
    x = "Sampling date",
    y = "Probability of grazer presence (%)"
  ) +
  theme_minimal() +
