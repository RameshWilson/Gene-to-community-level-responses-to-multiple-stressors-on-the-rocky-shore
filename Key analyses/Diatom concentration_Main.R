...
#     5) Whole-season Gaussian GAM + interaction table
...
#     Diatom concentration_Additional.R
...
# Fit Gaussian GLMs (identity link)
glm_JUN   <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = JUN,   family = gaussian(link = "identity"))
glm_JUL_1 <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = JUL_1, family = gaussian(link = "identity"))
glm_JUL_2 <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = JUL_2, family = gaussian(link = "identity"))
glm_AUG_1 <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = AUG_1, family = gaussian(link = "identity"))
glm_AUG_2 <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = AUG_2, family = gaussian(link = "identity"))
glm_SEP_1 <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = SEP_1, family = gaussian(link = "identity"))
glm_SEP_2 <- glm(Diatoms ~ Pad_Colour * Pad_Region, data = SEP_2, family = gaussian(link = "identity"))
...
extract_diatom_glm_stats <- function(model, date_label) {
  ...
  # Coefficient table (identity scale)
  ...
  # Direction labels (used only when CI fully outside band AND p<P_ALPHA)
  ...
  cs %>%
    ...
    dplyr::transmute(
      ...
      `Observed Prediction` = if_else(Term == "Warming × Pollution", round(obs_pred,  4), NA_real_),
...
diatom_glm_results_table <- purrr::map2_df(glm_models, DATE_LABELS, extract_diatom_glm_stats)

DT::datatable(
  diatom_glm_results_table,
  caption = "Diatoms (Gaussian GLM, identity): fixed effects & interaction summary",
...
brighton_data_plot <- brighton_data %>%
  ...
  dplyr::mutate(
    ...
    Treatment  = paste(Pad_Region, Pad_Colour, sep = " + "),
    Sampling_Date = factor(DATE_LABEL, levels = levels(all_preds$Sampling_Date))
  ) %>%
  dplyr::filter(!is.na(Sampling_Date))

# Plot
diatom_line_plot <- ggplot(all_preds, aes(x = Sampling_Date, y = predicted, group = Treatment)) +
  ...
  geom_jitter(
    ...
    aes(x = Sampling_Date, y = Diatoms, color = Treatment),
    shape = 20, width = 0.25, height = 0, alpha = 0.7, size = 1.5, inherit.aes = FALSE
  ) +
  labs(
    title = "Diatom concentration",
    x = "Sampling date",
    y = expression("Diatom concentration (" ~ plain("mg") * "/" * plain("cm")^2 * ")")
  ) +
  theme_minimal() +
  theme(
...
# Keep just one legend for panel plot
...
########## 6) Whole-season GAM (Gaussian–identity) ##########
...
print(summary(diatoms_gam_model))

########## 7) GAM parametric effects & interaction summary ##########

extract_diatoms_gam_model_stats <- function(model, date_label) {
  ...
  names(ptab) <- names(ptab) %>%
    tolower() %>%
    gsub("[[:punct:] ]+", "_", .)
  
  # p column name differs across mgcv versions (often "pr(>|t|)")
  ...
  # ±5% equivalence band around null prediction (identity units)
  tol     <- EQUIV_PCT * abs(null_pred)
  band_lo <- -tol
  band_hi <-  tol
  ...
  tibble::tibble(
    ...
    Estimate     = round(c(βw, βp, βint), 4),
    `Std. Error` = round(c(se_w, se_p, se_i), 4),
    `p-value`    = ifelse(
      is.na(c(pv_w, pv_p, pv_i)),
      NA,
      format.pval(c(pv_w, pv_p, pv_i), digits = 3, eps = 0.001)
    ),
    ...
    `Interaction 95% CI (identity)` = c(NA, NA, sprintf("[%.4f, %.4f]", int_lo, int_hi)),
    `±5% Band (identity)`           = c(NA, NA, sprintf("[%.4f, %.4f]", band_lo, band_hi)),
    `Interaction Class`             = c(NA, NA, int_class)
  )
}

diatoms_gam_results_table <- extract_diatoms_gam_model_stats(diatoms_gam_model, "Whole Season")

DT::datatable(
  diatoms_gam_results_table,
  caption = "Diatoms GAM (Gaussian identity): parametric effects & interaction summary",
  options = list(pageLength = 10, scrollX = TRUE)
)
...
date_breaks <- as.numeric(DATE_LOOKUP$Sampling_Date - start_date)

# Because the model contains s(Pad_ID), predict.gam requires a Pad_ID in newdata
dummy_pad <- brighton_data_GAM$Pad_ID[which(!is.na(brighton_data_GAM$Pad_ID))][1]

# Prediction grid across the full season
pred_grid <- tidyr::expand_grid(
  Days_Since_Start = seq(
    min(brighton_data_GAM$Days_Since_Start, na.rm = TRUE),
...
# Predict on response scale (identity, so response == link)
pr_resp <- predict(
  diatoms_gam_model,
  newdata = pred_grid,
  type    = "response",
...
gam_preds <- pred_grid %>%
  dplyr::mutate(
    predicted = pr_resp$fit,
    conf.low  = pr_resp$fit - 1.96 * pr_resp$se.fit,
    conf.high = pr_resp$fit + 1.96 * pr_resp$se.fit,
...
diatoms_gam_timeplot <- ggplot(gam_preds, aes(x = Days_Since_Start, y = predicted, group = Treatment)) +
  ...
  geom_point(
    data = gam_datepoints,
    aes(
      x = Days_Since_Start, y = predicted,
      fill = Treatment,
  ...
  geom_jitter(
    ...
    aes(x = Days_Since_Start, y = Diatoms, color = Treatment),
  ...
  scale_fill_manual(values = TREATMENT_COLORS,  breaks = LEGEND_ORDER) +
  labs(
    title = "Whole-season GAM: diatom concentration",
    x = "Sampling date",
    y = expression("Diatom concentration (" ~ plain("mg") * "/" * plain("cm")^2 * ")")
  ) +
  theme_minimal() +
